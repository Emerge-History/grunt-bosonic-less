        ...
        function (e, output) {
            ...
            try {
                scripts.each(function (i, script) {
                    if ($(this).attr('src') && $(this).attr('embed')) {
                        scriptDeps.push($(this).attr('src'));
                    } else if (!$(this).attr('src')) {
                        if (mainScript !== undefined) {
                            return cb(new Error('Only one <script> is permitted in a Web Component declaration'));
                        }
                ...
                if (template !== null) {
                    ...
                    var results = $("template *").filter(function () {
                        return /^part\-/i.test(this[0].name);
                    });
                    for (var i = 0; i < results.length; i++) {
                        $(results[i]).attr("kind", results[i].name);
                        results[i].name = "part";
                    }
                    template = $('template').html();
                }
                ...
                if (mainScript === undefined) {
                    return cb(new Error('A <script> is mandatory in a Web Component declaration'));
                }
